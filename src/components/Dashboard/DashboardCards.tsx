...
// Imágenes SVG extraídas de Figma
...
function IconCard({ variant }: { variant: "1" | "3" | "4" | "5" }) {
  if (variant === "1") {
    return <img src={imgVector1} alt="icono" className="w-6 h-6" />;
  }
  if (variant === "3") {
    return <img src={imgVector2} alt="icono" className="w-6 h-6" />;
  }
  return <img src={imgVector} alt="icono" className="w-6 h-6" />;
}
...
const DashboardCards: React.FC<DashboardCardsProps> = ({ role }) => {
  // Aquí puedes condicionar la vista según el rol
  if (role === "aprendiz") {
    return (
  ...
  // Vista para admin, coordinador e instructor
  return (
    <div className="w-[1000px] mx-auto my-12 bg-white rounded-[10px] overflow-hidden shadow-lg p-8">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        ...
        {/* Tarjeta Solicitudes sin asignar */}
        ...
        {/* Gráfica de asignaciones de aprendices */}
        <div className="w-80 h-96 bg-white rounded-xl shadow outline outline-1 outline-color-azure-84 relative flex flex-col items-center">
          ...
          <div className="absolute left-4 top-4 w-72 h-56 bg-white rounded-xl flex flex-col items-start overflow-hidden">
            {/* Aquí iría la gráfica real, puedes integrar una librería como Chart.js si lo deseas */}
            <div className="w-72 h-56 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400">Gráfica</div>
          </div>
        </div>
